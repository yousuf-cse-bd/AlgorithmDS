/**
 * @file QuickSortPivotHigherIndex.cpp
 * @author Md. Yousuf Ali (yousuf.cse17@gmail.com)
 * @brief The quick sort algorithm using for-loop and not support random numbers
 * @version 0.1
 * @date 2023-01-11
 * @since WednesDay; 04:01 PM
...
const partition(vector <int> &listInt, const ssize_t lowerIndex, const ssize_t higherIndex){
    int pivot {listInt.at(higherIndex)};
    ssize_t j {lowerIndex};
    for(ssize_t i {lowerIndex}; i < higherIndex; i++){
        if(listInt.at(i) < pivot){
